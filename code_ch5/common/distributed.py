...
def run_one_shot(
    ...
    In the one-shot baseline every worker only observes its local partition.
    Therefore the augmentation quantile grid must be generated from the local
    sample size ``part.n`` rather than from the full distributed sample size.
    After each worker finishes DA-CSQRNN, the host directly averages the worker
    QRNN parameter vectors and then predicts from the averaged model.
    """
