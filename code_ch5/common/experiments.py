...
def run_comparison_replication(
    ...
    include_daqrnn : bool
        Run the DAqrnn method. Set this to ``False`` for DeepQuantreg-only
        reruns.
    ...
    benchmark_huber_pred = None
    benchmark_huber_boot = None
    huber_bench_lower = None
    huber_bench_upper = None
    huber_bench_rows = []
    if include_daqrnn:
    ...
    save_npz(
        ...
        # DAqrnn (Huber)
        pred_daqrnn=np.array([]) if da_huber is None else da_huber.final_predictions,
        pred_daqrnn_iter=np.array([]) if da_huber is None else da_huber.iteration_predictions,
        lower_daqrnn=np.array([]) if da_huber is None else da_huber.lower,
        upper_daqrnn=np.array([]) if da_huber is None else da_huber.upper,
        ...
        pred_benchmark_daqrnn=np.array([]) if benchmark_huber_pred is None else benchmark_huber_pred,
        pred_benchmark_daqrnn_boot=np.array([]) if benchmark_huber_boot is None else benchmark_huber_boot,
        lower_benchmark_daqrnn=np.array([]) if huber_bench_lower is None else huber_bench_lower,
        upper_benchmark_daqrnn=np.array([]) if huber_bench_upper is None else huber_bench_upper,
        # DeepQuantreg
    ...
    fit_rows = ([] if da_cs is None else list(da_cs.fit_rows)) + ([] if da_huber is None else list(da_huber.fit_rows)) + list(bench_rows)
    for row in fit_rows:
