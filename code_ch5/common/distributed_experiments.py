...
def run_distributed_replication(
    ...
    partitions = partition_dataset(train, K, rng) if include_dcs or include_one_shot else []
    dcs = None
    if include_dcs:
        dcs = run_dcs_qrnn_c(
            partitions,
    ...
    save_npz(
        ...
        true_quantiles=true_q,
        tau_grid=tau_grid,
        one_shot_worker_n=np.array([part.n for part in partitions], dtype=int),
        one_shot_tau_grid_n=np.array([len(make_tau_grid(part.n)) for part in partitions], dtype=int),
        ...
        pred_dcs=np.array([]) if dcs is None else dcs.final_predictions,
        pred_dcs_iter=np.array([]) if dcs is None else dcs.iteration_predictions,
        lower_dcs=np.array([]) if dcs is None else dcs.lower,
        upper_dcs=np.array([]) if dcs is None else dcs.upper,
        pred_one_shot=np.array([]) if one_shot_pred is None else one_shot_pred,
        tau_schedule_dcs=np.array([]) if dcs is None else dcs.tau_schedule,
    )
